...
正式版安装包发布在 [GitHub Releases](https://github.com/Gliese-876/Crid/releases)。当前首个正式版是 `v1.0.0-release`。

### Android

下载 `Crid-1.0.0-release-android.apk` 后直接安装即可。如果系统提示不允许安装未知来源应用，请在 Android 设置中为浏览器或文件管理器开启“安装未知应用”权限。

也可以按设备架构下载体积更小的分包：

- `Crid-1.0.0-release-android-arm64.apk`：大多数近年的 Android 手机
- `Crid-1.0.0-release-android-armeabi-v7a.apk`：较老的 32 位 Android 设备
- `Crid-1.0.0-release-android-x86_64.apk`：x86_64 模拟器或少数 x86_64 设备

`Crid-1.0.0-release-android.aab` 主要用于应用商店分发，普通用户不需要下载它。
